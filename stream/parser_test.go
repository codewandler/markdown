...
func TestInlineSubset(t *testing.T) {
	...
	assertContains(t, got, eventView{Kind: EventText, Text: "link", Style: InlineStyle{LinkData: &LinkData{HasLink: true, Link: "https://example.com"}}})
}
...
func TestGFMInlineExtensions(t *testing.T) {
	...
	assertContains(t, events, eventView{Kind: EventText, Text: "https://example.com", Style: InlineStyle{LinkData: &LinkData{HasLink: true, Link: "https://example.com"}}})
	assertContains(t, events, eventView{Kind: EventText, Text: "www.example.org/path", Style: InlineStyle{LinkData: &LinkData{HasLink: true, Link: "http://www.example.org/path"}}})
	assertContains(t, events, eventView{Kind: EventText, Text: "foo@bar.example.com", Style: InlineStyle{LinkData: &LinkData{HasLink: true, Link: "mailto:foo@bar.example.com"}}})
}

func TestImageSubset(t *testing.T) {
	inline := parseAll(t, "![foo](https://example.com/logo.png)\n")
	got := viewEvents(inline)
	assertContains(t, got, eventView{Kind: EventText, Text: "foo", Style: InlineStyle{Image: true, LinkData: &LinkData{HasLink: true, Link: "https://example.com/logo.png"}}})
	ref := parseAll(t, "[bar]: /bar.png\n\n![bar]\n")
	got = viewEvents(ref)
	assertContains(t, got, eventView{Kind: EventText, Text: "bar", Style: InlineStyle{Image: true, LinkData: &LinkData{HasLink: true, Link: "/bar.png"}}})
}
...
func TestLinkReferenceDefinitions(t *testing.T) {
	t.Run("multiline definition resolves later reference", func(t *testing.T) {
		...
		assertContains(t, events, eventView{Kind: EventText, Text: "foo", Style: InlineStyle{LinkData: &LinkData{HasLink: true, Link: "/url", LinkTitle: "title"}}})
	})
	...
	t.Run("next non-title line resolves after pending destination", func(t *testing.T) {
		...
		assertContains(t, events, eventView{Kind: EventText, Text: "foo", Style: InlineStyle{LinkData: &LinkData{HasLink: true, Link: "/url"}}})
	})
}
